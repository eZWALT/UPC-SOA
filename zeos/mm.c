...
SharedPage shared_pages[NUM_SHARED_PAGES];
...
// array of the page directory for each process
page_table_entry dir_pages[NR_TASKS][TOTAL_PAGES]
    __attribute__((__section__(".data.task")));

// array of the page table for each process
...
/* Allocates NUM_SHARED_PAGES frames and stores their references */
void init_shared_pages(){
    for (int idx = 0; idx < NUM_SHARED_PAGES; ++idx){
        shared_pages[idx].frame = alloc_frame();
        shared_pages[idx].num_refs = 0;
        shared_pages[idx].to_clear = 0;
...
int is_cow_page(page_table_entry* pte, unsigned page){
    return pte[page].entry != 0 && pte[page].bits.rw == 0;
}
